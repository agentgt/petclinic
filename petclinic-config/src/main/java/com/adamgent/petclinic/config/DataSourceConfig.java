...
public record DataSourceConfig(
	...
	public String toString() {
		/*
		 * This is to avoid toStringing the password.
		 * This should probably be put in some util class.
		 */
	...
	public DataSource dataSource() {
		...
		for (int i = 1; i < retryCount; i++) {
			try {
				return hikari(url, username, password, driverClassName);
			}
			catch (RuntimeException e) {
				int sleep = i == 1 ? initialSleep : retrySleep;
				logger.log(
						Level.WARNING,
						"Retrying to connect to db. "
								+ "retryCount: "
								+ retryCount
								+ ", try: "
								+ i
